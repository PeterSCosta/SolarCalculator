...
const EditCost: React.FC = () => {
  ...
  const [editingCost, setEditingCost] = useState<Cost>();

  useEffect(() => {
    api.get(`v1/costs/${id}`).then(response => {
      setEditingCost(response.data);
    });
  }, [id]);

  const handleSubmit = useCallback(
    async (data: EditCostFormData) => {
      try {
        ...
        await api.put(`/v1/costs/${id}`, data);
  ...
  return (
    <Container>
      ...
      <Content>
        <h1>Editar custo do estado {editingCost?.state}</h1>
